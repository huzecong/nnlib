...
__all__ = ['timestamp', 'Logging']


def timestamp(path=False):
    if not path:
...
class Logging:
    ...
    def info(cls, *args, **kwargs):
        Logging(cls.INFO, file=sys.stderr)._log(*args, prefix="INFO:", prefix_color='green', **kwargs)

    @classmethod
    def verbose(cls, *args, **kwargs):
        Logging(cls.VERBOSE, file=sys.stderr)._log(*args, **kwargs)
